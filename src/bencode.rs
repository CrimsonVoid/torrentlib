...
impl Benc {
    ...
    fn string<R>(bytes: &mut io::Bytes<R>, c: u8) -> error::Result<Vec<u8>>
        ...
        for c in bytes.by_ref() {
            match c {
                Ok(c@b'0'...b'9') =>
                    match len.checked_mul(10).and_then(|n| n.checked_add((c - b'0') as usize)) {
                        Some(n) => len = n,
    ...
    fn int<R>(bytes: &mut io::Bytes<R>) -> error::Result<i64>
        ...
        let neg = match bytes.next() {
            Some(Ok(b'-'))          => -1,
            Some(Ok(c@b'0'...b'9')) => { num = (c - b'0') as i64; 1 }
            Some(Ok(_)) | None      => return err,
            Some(Err(e))            => return Err(error::Error::from(e)),
        };

        if neg == -1 { // 1..9 must follow -
            match bytes.next() {
                Some(Ok(c@b'1'...b'9')) => { num = (c - b'0') as i64; },
                Some(Err(e))            => return Err(error::Error::from(e)),
                _                       => return err,
            }
        } else if num == 0 { // No digits may follow 0
            match bytes.next() {
        ...
        for c in bytes {
            match c {
                Ok(c@b'0'...b'9') =>
                    match num.checked_mul(10).and_then(|n| n.checked_add((c - b'0') as i64)) {
                        Some(n) => num = n,
                        None    => return Err(error::Error::Other("Integer overflow"))
                    },
    ...
    fn dict<R>(bytes: &mut io::Bytes<R>) -> error::Result<HashMap<Vec<u8>, Benc>>
        ...
        loop {
            let key = match Benc::node(bytes, Some(b'e')) {
                ...
                Ok(_)                       => return Err(error::Error::Other(
...
impl convert::From<String> for Benc {
    fn from(s: String) -> Benc { Benc::String(s.into_bytes()) }
}

impl convert::From<Vec<u8>> for Benc {
    fn from(s: Vec<u8>) -> Benc { Benc::String(s) }
}

impl convert::From<i64> for Benc {
    fn from(s: i64) -> Benc { Benc::Int(s) }
}

impl convert::From<Vec<Benc>> for Benc {
    fn from(s: Vec<Benc>) -> Benc { Benc::List(s) }
}

impl convert::From<HashMap<Vec<u8>, Benc>> for Benc {
    fn from(s: HashMap<Vec<u8>, Benc>) -> Benc { Benc::Dict(s) }
}
...
mod test_nodetype {
    ...
    fn type_of() {
        ...
        for c in vec![(b'i', NodeType::Int), (b'l', NodeType::List), (b'd', NodeType::Dict)] {
...
mod test_benc {
    ...
    fn string() {
        fn is_valid(data: &str, first: u8) {
            ...
            assert(|brd| Benc::string(brd, first),
                data.as_bytes().bytes(),
                Ok(bytes!(expect)),
            );
        }
    ...
    fn int() {
        fn is_valid(expect: i64) {
            assert(Benc::int, format!("{}e", expect).as_bytes().bytes(), Ok(expect));
        }

        fn is_invalid(data: &str) {
            assert(Benc::int, data.as_bytes().bytes(), Err(error::Error::Other("Mock data")));
        }
    ...
    fn list() {
        ...
        assert(Benc::list, b"5:helloi4e".bytes(), Err(error::Error::Other("Mock data")));
    }
    ...
    fn dict() {
        ...
        assert(Benc::dict,
            concat!(
                ...
                "i3ee7:yahallo2::)ee3:str5:helloe").as_bytes().bytes(),
            Ok(hashmap!(
        ...
        assert(Benc::dict, b"2:hi5:hello1:ai32ee".bytes(), Err(error::Error::Other(("Mock data"))));
    }
...
mod bench {
    ...
    fn list(b: &mut test::Bencher) {
        let data = concat!(
            ...
            "e7:yahallo2::)eed2:hi5:hello3:inti15eee").as_bytes();
    ...
    fn dict(b: &mut test::Bencher) {
        let data = concat!(
            ...
            "1ei2ei3ee7:yahallo2::)ee3:str5:helloe").as_bytes();
